...
class StatusmonitorTaskAdditionalFieldProvider implements \TYPO3\CMS\Scheduler\AdditionalFieldProviderInterface
{
        ...
        \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $parentObject)
    {
        ...
        $additionalFields['statusmonitorUsername'] = [
            'code' => '<input type="text" class="form-control" name="tx_scheduler[statusmonitorUsername]" value="' .
                 $taskInfo['statusmonitorUsername'] . '">',
                'label' => $this->getLanguageService()->sL(
                    'LLL:EXT:form4_statusmonitor/Resources/Private/Language/locallang_db.xlf:task.statusmonitor.statusmonitorUsername'),
                'cshKey' => '',
                'cshLabel' => ''
        ];
        
        $additionalFields['statusmonitorPassword'] = [
            'code' => '<input type="password" class="form-control" name="tx_scheduler[statusmonitorPassword]" value="' .
                 $taskInfo['statusmonitorPassword'] . '">',
                'label' => $this->getLanguageService()->sL(
                    'LLL:EXT:form4_statusmonitor/Resources/Private/Language/locallang_db.xlf:task.statusmonitor.statusmonitorPassword'),
                'cshKey' => '',
                'cshLabel' => ''
        ];
        
        $additionalFields['statusmonitorPostUrl'] = [
            'code' => '<input type="text" class="form-control" name="tx_scheduler[statusmonitorPostUrl]" value="' .
                 $taskInfo['statusmonitorPostUrl'] . '">',
                'label' => $this->getLanguageService()->sL(
                    'LLL:EXT:form4_statusmonitor/Resources/Private/Language/locallang_db.xlf:task.statusmonitor.statusmonitorPostUrl'),
                'cshKey' => '',
                'cshLabel' => ''
        ];
        ...
        if (! isset($submittedData['statusmonitorPostUrl']) || empty($submittedData['statusmonitorPostUrl'])) {
            $parentObject->addMessage(
                $this->getLanguageService()
                    ->sL('LLL:EXT:form4_statusmonitor/Resources/Private/Language/locallang_db.xlf:error.noUrl'),
                FlashMessage::ERROR);
            $result = false;
        }
        return $result;
